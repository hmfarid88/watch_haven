<%-- 
    Document   : symvouccher
    Created on : Dec 18, 2017, 12:45:19 AM
    Author     : user
--%>
...
<html class="no-js" lang="">
        ...
        <nav style="margin: 0 auto" class="navbar navbar-inverse">
            ...
            <div class="collapse navbar-collapse" id="myNavbar">
                <ul class="nav navbar-nav">
                    ...
                    <li> <a href="Vendor_sale.jsp"><span class="fa fa-window-restore"></span> New-Sale</a></li>
                    <!--<li> <a href="vendor_saleinfo_edit_date.jsp"><span class="fa fa-edit"></span>  Edit-Date</a></li>-->
                                ...
                                <tr style=" border-bottom-style: hidden">
                                    <td style="border-right-style: hidden;font-size: 12px">
                                        ...
                                        <%
                                            try {
                                                con = Ssymphonyy.getConnection();
                                                String query = "select C_NAME, PHONE_NUMBER, ADDRESS from customerinfo where CUSTOMER_ID=(select max(CUSTOMER_ID) from customerinfo)";
                                                ps = con.prepareStatement(query);
                                                rs = ps.executeQuery();
                                                while (rs.next()) {
                            ...
                            <table border="1" style=" min-height: 290px" width="100%" class="myTables" >
                                ...
                                <tbody>
                                    ...
                                    <%
                                        try {
                                            con = Ssymphonyy.getConnection();
                                            String query = "select  PRODUCT_ID, BRAND, MODEL, COLOR, COST_PRICE from vendor_sale where  CUSTOMER_ID=(select max(CUSTOMER_ID) from customerinfo)";
                                            ps = con.prepareStatement(query);
                                            rs = ps.executeQuery();
                                            while (rs.next()) {
                                    %>
   ...
   try { if (con != null) con.close(); con=null; } catch (SQLException ex2) { }
                                ...
                                <%
                                    try {
                                        con = Ssymphonyy.getConnection();
                                        String query = "select  PRODUCT_NAME, PRODUCT_ID, MODEL, COST_PRICE from vendor_accessor_sale where  CUSTOMER_ID=(select max(CUSTOMER_ID) from customerinfo)";
                                        ps = con.prepareStatement(query);
                                        rs = ps.executeQuery();
                                        while (rs.next()) {
                                %>
                                    ...
                                    <%
                                        try {
                                            ...
                                            String query = "select count(PRODUCT_ID) as pqnty, sum(COST_PRICE)as ptotal "
                                                    + " from vendor_sale where  CUSTOMER_ID=(select max(CUSTOMER_ID) from customerinfo)";
                                            ps = con.prepareStatement(query);
                                            rs = ps.executeQuery();
                                            while (rs.next()) {
                                    ...
                                    <%
                                        try {
                                            ...
                                            String query = "select count(PRODUCT_ID) as aqnty, sum(COST_PRICE)as atotal "
                                                    + " from vendor_accessor_sale where  CUSTOMER_ID=(select max(CUSTOMER_ID) from customerinfo) ";
                                            ps = con.prepareStatement(query);
                                            rs = ps.executeQuery();
                                            while (rs.next()) {
