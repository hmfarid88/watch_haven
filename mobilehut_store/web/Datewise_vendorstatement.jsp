                                        ...
                                        <tbody id="myTable">
                                            <%
                                                ...
                                                try {
                                                    ...
                                                    String date="select DATE from vendor_stock where VENDOR='" + vendor + "' and DATE between '"+ date1 +"' and '"+ date2 +"'"
                                                            + "UNION select DATE from cashpayment where PAYMENT_NAME='" + vendor + "' and DATE between '"+ date1 +"' and '"+ date2 +"' order by DATE";
                                                    ps=con.prepareStatement(date);
                                                    rs = ps.executeQuery();
                                                        while (rs.next()) {
                                                        String datee=rs.getString("DATE");
                                                        String qty="select  count(*) as qty, sum(PURCHASE_PRICE) as value from vendor_stock  where VENDOR='" + vendor + "' and DATE='"+ datee +"'";
                                                        ps=con.prepareStatement(qty);
                                                        rs1 = ps.executeQuery();
                                                        rs1.next();
                                                        String payment="select  sum(AMOUNT) as ttamount from cashpayment  where PAYMENT_NAME='" + vendor + "' and DATE='"+ datee +"'";
                                                        ps=con.prepareStatement(payment);
                                                        rs2 = ps.executeQuery();
                                                        rs2.next();
                                                        String total="select sum(PURCHASE_PRICE) from vendor_stock  where VENDOR='" + vendor + "' and DATE<='"+ datee +"'";
                                                        ps=con.prepareStatement(total);
                                                        rs3 = ps.executeQuery();
                                                        rs3.next();
                                                        String balance="select  sum(AMOUNT) from cashpayment  where PAYMENT_NAME='" + vendor + "' and DATE<='"+ datee +"'";
