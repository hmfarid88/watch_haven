 ...
 * @author Acer
 */
public class A_sale_replace_Servlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
       ...
       PrintWriter out = response.getWriter();
            ...
            try {
                ...
                String query = "select count(*) as imeei from accessoriesstock where  PRODUCT_ID=?";
                ps = con.prepareStatement(query);
                ps.setString(1, newime);
                rs0 = ps.executeQuery();
                rs0.next();
                int a = rs0.getInt("imeei");
                String queryy = "select count(*) as imeii from accessor_sale where  PRODUCT_ID=?";
                ...
                if (a>0 && b>0) {
                    String soldimei = "select count(*) as imeei from accessoriesstock where  PRODUCT_ID=?";
                ...
                String query0 = "insert into accessoriesstock (PRODUCT_NAME, MODEL, PRODUCT_ID, COST_PRICE, SELL_PRICE, VENDOR, DATE) select "
                        + " PRODUCT_NAME, MODEL, PRODUCT_ID, COST_PRICE, STOCK_RATE, VENDOR, STOCK_DATE from accessor_sale where PRODUCT_ID=?";
                ps = con.prepareStatement(query0);
                ps.setString(1, soldime);
                int p = ps.executeUpdate();
                if (p>0) {
                    String query01="select  PRODUCT_NAME, MODEL, PRODUCT_ID, COST_PRICE, SELL_PRICE, VENDOR, DATE from accessoriesstock where PRODUCT_ID=?";
                    ...
                    String query1 = "update  accessor_sale set PRODUCT_NAME=?, PRODUCT_ID=?, MODEL=?,  COST_PRICE=?, STOCK_RATE=?, SELL_PRICE=?, DISCOUNT=?, DIS_NOTE=?, VENDOR=?, STOCK_DATE=?  where PRODUCT_ID=?";
                    ...
                    ps.setString(8, disnote);
                    ps.setString(9, vendor);
                    ps.setString(10, date);
                    ps.setString(11, soldime);
                    ps.executeUpdate();
